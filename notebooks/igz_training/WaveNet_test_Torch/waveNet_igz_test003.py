...
def preprocess_audio_in_memory(example, desired_length=5.0, target_sr=22050):
    ...
    for event in detected_events:
        ...
        # Ensure the resampled snippet is of the desired length
        resampled_length = int(desired_length * target_sr)
        if len(snippet_resampled) < resampled_length:
            padding = resampled_length - len(snippet_resampled)
            snippet_resampled = np.pad(snippet_resampled, (0, padding), 'constant')
...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # Remove excess padding to match the input length

...
from .components import XCEventMapping
from .components.transforms import BirdSetTransformsWrapper
from birdset.utils import pylogger
from .datamodule_configs import DatasetConfig, LoadersConfig
...
class BaseDataModuleHF(L.LightningDataModule):
    ...
    def num_classes(self):
        return len(
            datasets.load_dataset_builder(self.dataset_config.hf_path, self.dataset_config.hf_name).info.features[
                "ebird_code"].names)
    ...
    def _save_dataset_to_disk(self, dataset: Dataset | DatasetDict):
        ...
        fingerprint = dataset[next(iter(dataset))]._fingerprint if isinstance(dataset,
                                                                              DatasetDict) else dataset._fingerprint  # changed to next_iter to be more robust

...
from .components.event_mapping import XCEventMapping
...
class LoaderConfig:
    batch_size: int = 32
    shuffle: bool = True  # Example attribute


@dataclass
class LoadersConfig:
    train: Optional[LoaderConfig] = None
    valid: Optional[LoaderConfig] = None
    test: Optional[LoaderConfig] = None

    def __post_init__(self):
        # Initialize mutable defaults using default_factory
        if self.train is None:
            self.train = LoaderConfig()
        if self.valid is None:
            self.valid = LoaderConfig(shuffle=False)
        if self.test is None:
            self.test = LoaderConfig(shuffle=False)
